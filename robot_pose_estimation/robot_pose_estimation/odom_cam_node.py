...
class RobotPoseEstimation(Node):
    def __init__(self):
        ...
        self.joint_pub = self.create_publisher(JointState, '/joint_states', 10) # Só para vizualização
    ...
    def timer_callback(self):
        ...
        # JointState (para o robot_state_publisher) - Apenas para vizualização
        joint_state = JointState()
        joint_state.header = Header()
        joint_state.header.stamp = self.get_clock().now().to_msg()
        joint_state.name = ['left_wheel_joint', 'right_wheel_joint']
        joint_state.velocity = [0.0, 0.0]
        joint_state.position = [0.0, 0.0]  # posE(rad),posD(rad)
